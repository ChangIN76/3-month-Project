...
// 기본 매개 변수를 지정하기 위해서는 오른쪽에서부터 지정해주어야 합니다.
...
#pragma region 범위 지정 연산자
	// 여러 범위에서 사용되는 식별자를 구분하는 데 사용되는 연산자입니다.

	// char character = 'A';

	// std::cout << "지역 변수 charactor 변수의 값 : " << character << std::endl;

	// 범위 지정 연산자는 전역 변수와 같은 이름의 지역 변수가 선언되었을
	// 때 가장 가까운 범위에 선언된 변수의 이름을 사용하는 범위 규칙이
	// 존재하기 때문에 전역 변수가 호출되지 않습니다.

	// std::cout << "전역 변수 charactor 변수의 값 : " << ::character << std::endl;
